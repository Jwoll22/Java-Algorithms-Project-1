...
public class MyDictionary implements DictInterface
{
	...
	{
		boolean doneIn, doneOut, charMatch, prefix, word, possible;
		String curr;
		doneOut = false;
		prefix = false;
		word = false;
		...
		for (int i = 0; i < list.size() && !doneOut; i++)
		{
			...
			for (int j = start; j <= end && !doneIn; j++)
			{
				...
				{
					charMatch = (s.charAt(j) == curr.charAt(j));
					if (charMatch && j == end && jj < curr.length()-1)
					{
						prefix = true;  // Valid prefix
						doneIn = true;
					}
					else if (charMatch && j == end && jj == curr.length()-1)
					{
						word = true;    // Valid word
			...
			if (prefix && curr.length() == size)
			{
				doneOut = true;
				possible = true; //Also a valid word is POSSIBLE with given length
			}
		}
